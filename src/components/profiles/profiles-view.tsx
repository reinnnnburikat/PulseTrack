...
export function ProfilesView() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['profiles'] }); setShowCreate(false); resetForm() },
  })
  ...
  const resetForm = () => setForm({ name: '', active_duration: '25', rest_duration: '5', cycles: '4', infinite_cycles: false, tone: 'teasing', intensity_mode: false })
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => { resetForm(); setShowCreate(true) }} className="bg-primary text-xs">
            ...
            >
              <Card className="bg-card/60 border-border/50 hover:border-primary/30 transition-colors group">
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        <Badge variant="outline" className={`text-[10px] ${toneColors[p.tone as Tone] || ''}`}>
                          {p.tone}
                        </Badge>
                        {p.intensity_mode && (
                  ...
                  <Button size="sm" className="w-full bg-primary/10 text-primary hover:bg-primary/20 text-xs"
        ...
        <DialogContent className="bg-card border-border/50">
          ...
          <DialogFooter>
            ...
            <Button onClick={() => editProfile && updateMutation.mutate({
              ...editProfile,
              name: form.name,
              active_duration: parseInt(form.active_duration),
              rest_duration: parseInt(form.rest_duration),
              cycles: parseInt(form.cycles),
              infinite_cycles: form.infinite_cycles,
              tone: form.tone,
              intensity_mode: form.intensity_mode,
