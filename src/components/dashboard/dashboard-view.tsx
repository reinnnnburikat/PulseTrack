...
import { useAuthStore, useGamificationStore, useSettingsStore } from '@/store/auth-store'
import { motion } from 'framer-motion'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Progress } from '@/components/ui/progress'
import { Badge } from '@/components/ui/badge'
import { Timer, Brain, BookmarkPlus, Flame, Zap, TrendingUp, Clock, Trophy, ArrowRight } from 'lucide-react'
...
export function DashboardView() {
  ...
  const stats = [
    ...
    { label: `Level ${gam.level}`, value: `${Math.floor(gam.xp / 500) * 500 + 500 - gam.xp} XP`, icon: Trophy, color: 'text-violet-400' },
  ]
  ...
  const getInsight = () => {
    ...
    return { text: 'Start your first session today to build your streak.', icon: '💓' }
  }
            ...
            <Card className="bg-card/60 border-border/50">
              <CardContent className="p-4">
                ...
                {stat.label.startsWith('Level') && (
                  <Progress value={xpProgress} className="mt-2 h-1.5 bg-secondary" />
                )}
              </CardContent>
            </Card>
          </motion.div>
